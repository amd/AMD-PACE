  ...
  [&] {                                                                     \
    switch (HEAD_DIM) {                                                     \
      ...
      default: {                                                            \
        TORCH_CHECK(                                                        \
            false,                                                          \
            "Invalid CPU attention head_dim: " + std::to_string(HEAD_DIM)); \
      }                                                                     \
    }                                                                       \
  }()
  ...
  [&] {                                                                   \
    switch (ISA_TYPE) {                                                   \
      case cpu_attention::ISA::VEC: {                                     \
        using attn_impl = cpu_attention::                                 \
            AttentionImpl<cpu_attention::ISA::VEC, scalar_t, head_dim>;   \
        return __VA_ARGS__();                                             \
      }                                                                   \
      case cpu_attention::ISA::VEC16: {                                   \
        using attn_impl = cpu_attention::                                 \
            AttentionImpl<cpu_attention::ISA::VEC16, scalar_t, head_dim>; \
        return __VA_ARGS__();                                             \
      }                                                                   \
      default: {                                                          \
...
at::Tensor get_scheduler_metadata(
  ...
  TORCH_CHECK(query_start_loc.is_cpu(), "query_start_loc must be on CPU");
  ...
  TORCH_CHECK(causal, "Only causal masking is supported.");
...
void cpu_attn_reshape_and_cache(
    const at::Tensor& key, // [token_num, head_num, head_size]
    const at::Tensor& value, // [token_num, head_num, head_size]
    at::Tensor& key_cache, // [num_blocks, num_kv_heads, block_size, head_size]
    at::Tensor&
...
void cpu_attention_with_kv_cache(
    const at::Tensor& query, // [num_tokens, num_heads, head_size]
    const at::Tensor&
        key_cache, // [num_blocks, num_kv_heads, block_size, head_size]
    const at::Tensor&
        value_cache, // [num_blocks, num_kv_heads, block_size, head_size]
    at::Tensor& output, // [num_tokens, num_heads, head_size]
    const at::Tensor& query_start_loc, // [num_tokens + 1]
    const at::Tensor& seq_lens, // [num_tokens]
    const double scale,
    const bool causal,
    const c10::optional<at::Tensor>& alibi_slopes, // [num_heads]
    const int64_t sliding_window_left,
    const int64_t sliding_window_right,
    const at::Tensor& block_table, // [num_tokens, max_block_num]
    const double softcap,
    const at::Tensor& scheduler_metadata,
    const c10::optional<at::Tensor>& s_aux // [num_heads]
) {
  ...
  TORCH_CHECK(block_table.is_cpu(), "block_table must be on CPU");
  TORCH_CHECK(
      scheduler_metadata.is_contiguous(),
      "scheduler_metadata must be contiguous");
  TORCH_CHECK(scheduler_metadata.is_cpu(), "scheduler_metadata must be on CPU");
