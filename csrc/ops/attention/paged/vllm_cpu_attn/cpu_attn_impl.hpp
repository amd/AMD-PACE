 ...
 * All rights reserved.
 *
 * This source code is derived from vLLM (https://github.com/vllm-project/vllm)
...
class AttentionScratchPad {
 public:
  ...
  void update(
      ...
      const int64_t q_buffer_elem_size,
      const int64_t logits_buffer_elem_size,
      const int64_t output_buffer_elem_size,
      const int64_t max_num_q_per_iter,
      const int64_t q_head_tile_size,
    ...
    buffer_offset += calcu_logits_buffer_size(
        max_num_q_per_iter, kv_tile_size, logits_buffer_elem_size);
  ...
  float* get_logits_buffer() {
    return reinterpret_cast<float*>(
        thread_scratchpad_ptr + logits_buffer_offset_);
  }

  float* get_output_buffer() {
    return reinterpret_cast<float*>(
        thread_scratchpad_ptr + output_buffer_offset_);
  }
  ...
  volatile bool* get_reduce_flag_buffer() {
    return reinterpret_cast<volatile bool*>(
        reduction_scratchpad_ptr + reduce_flag_buffer_offset_);
  }

  float* get_reduce_output_buffer() {
    return reinterpret_cast<float*>(
        reduction_scratchpad_ptr + reduce_output_buffer_offset_);
  }

  float* get_reduce_max_buffer() {
    return reinterpret_cast<float*>(
        reduction_scratchpad_ptr + reduce_max_buffer_offset_);
  }

  float* get_reduce_sum_buffer() {
    return reinterpret_cast<float*>(
        reduction_scratchpad_ptr + reduce_sum_buffer_offset_);
  }
 ...
 private:
  ...
  static int64_t calcu_q_buffer_size(
  ...
  static int64_t calcu_logits_buffer_size(
      const int64_t max_num_q_per_iter,
  ...
  static int64_t calcu_partial_output_buffer_size(
      const int64_t q_tile_size,
      const int64_t head_dim,
      const int64_t elem_size) {
    return round_to_64(q_tile_size * head_dim * elem_size);
  ...
  static int64_t calcu_reduce_output_buffer_size(
      ...
      const int64_t output_buffer_elem_size) {
    return round_to_64(
        total_split_num * q_head_tile_size * head_dim *
 ...
 public:
  ...
  torch::Tensor schedule(const ScheduleInput& input) const {
    ...
    for (int32_t req_id = 0; req_id < input.num_reqs; ++req_id) {
      ...
      const int32_t q_start_pos = (causal ? (seq_len - q_token_num) : 0);
      ...
      for (int32_t token_id = 0; token_id < q_token_num;
        ...
        const auto [kv_tile_pos_left, kv_tile_pos_right] = calcu_kv_tile_pos(
            ...
            q_tile_pos_left,
            q_tile_pos_right,
            left_sliding_window_size,
            right_sliding_window_size);
        const auto [aligned_kv_tile_pos_left, aligned_kv_tile_pos_right] =
            align_kv_tile_pos(
                kv_tile_pos_left, kv_tile_pos_right, kv_len_alignment);
    ...
    for (int32_t req_id = 0; req_id < input.num_reqs; ++req_id) {
      ...
      for (int32_t token_id = 0; token_id < q_token_num;
        ...
        while (curr_kv_len > 0) {
          ...
          int32_t spilt_size = std::min(
              std::max(remaining_kv_len, (int64_t)min_split_kv_len),
              (int64_t)curr_kv_len);
          curr_workitem = AttentionWorkItemGroup(
              req_id,
              token_id,
          ...
          curr_workitem = AttentionWorkItemGroup(
              req_id, token_id, kv_token_pos_start, seq_len);
    ...
    int64_t metadata_tensor_size = sizeof(AttentionMetadata) +
    ...
    AttentionMetadata* metadata_ptr =
        new (metadata_tensor.data_ptr()) AttentionMetadata(
            input.isa,
            workitems.size(),
            reduce_workitems.size(),
            cum_split_num,
            split_kv_q_token_num_threshold);
    AttentionWorkItemGroup* workitem_groups_ptr =
    ...
    std::memcpy(
        metadata_ptr->cu_workitem_num_per_thread + 1,
...

    {
      ...
      sc.update(
          0,
          metadata_ptr->reduction_split_num,
          input.head_dim,
    ...
    PaceScratchPadManager::get_scratchpad_manager()->realloc(scratchpad_size);
  ...
  static int64_t calcu_default_tile_size(
      int64_t cache_size,
      int64_t head_dim,
      int64_t elem_size,
    ...
    int64_t tile_size = cache_size /
        (head_dim *
             (q_buffer_elem_size + 2 * elem_size + output_buffer_elem_size) +
    ...
    void operator()(DEFINE_CPU_ATTENTION_PARAMS) {
      ...
      // compute Q@K logits
      {
        ...
        for (int32_t block_idx = start_block_idx; block_idx < end_block_idx;
          ...
          kv_cache_t* k_cache_block_ptr = k_head_cache_ptr +
          ...
          for (int32_t block_group_idx = 0;
               block_group_idx < curr_group_num_in_block;
               ++block_group_idx) {
              ...
              for (int32_t i = 0; i < q_head_num; ++i) {
                ...
                vec_op::unroll_loop<int32_t, vec_num>([&](int32_t vec_idx) {
                  vec_op::FP32Vec16 vec(
                      scale_curr_logits_buffer + vec_idx * 16);
                  vec = vec * scale_vec;
      ...
      // compute P@V
      {
        ...
        int64_t prob_buffer_stride = kv_tile_token_num *
        ...
        for (int32_t block_idx = start_block_idx; block_idx < end_block_idx;
             ++block_idx) {
          int32_t physical_block_idx = block_table[block_idx];
          kv_cache_t* v_cache_block_ptr = v_head_cache_ptr +
              physical_block_idx * kv_cache_num_blocks_stride +
              curr_group_offset;
          curr_group_num_in_block =
              std::min(remaining_group_num, curr_group_num_in_block);
          ...
          for (int32_t head_dim_group_idx = 0;
               head_dim_group_idx < head_dim_group_num;
               ++head_dim_group_idx) {
...
#pragma omp parallel for schedule(static, 1)
    for (int thread_id = 0; thread_id < thread_num; ++thread_id) {
      ...
      // init buffers
      void* scratchpad_ptr =
          PaceScratchPadManager::get_scratchpad_manager()->get_data<void>();
        ...
        if (task_idx < workitem_groups_counter_num) {
          ...
          for (int32_t workitem_group_idx = 0;
            ...
            const int32_t q_start_pos =
                (causal ? seq_len - (q_end - q_start) : 0);
            ...
            for (int32_t q_token_offset = 0; q_token_offset < q_token_num;
              ...
              int32_t curr_kv_head_idx = use_gqa
                  ? kv_head_idx
                  : (kv_head_idx / q_heads_per_kv); // for GQA disabled case
              ...
              for (int32_t kv_tile_pos = rounded_kv_tile_start_pos;
                ...
                for (int32_t q_head_tile_token_offset = 0;
                  ...
                  const int32_t q_tile_token_num = std::min(
    ...
    for (int32_t split_idx = 0; split_idx < split_num; ++split_idx) {
      ...
      if (split_idx > 0) {
        ...
        for (int32_t head_idx = 0; head_idx < curr_head_num; ++head_idx) {
          ...
          float* __restrict__ non_scale_output_iter = final_max > curr_max
              ? curr_output_buffer
              : curr_split_output_buffer_iter;
          float* __restrict__ scale_output_iter = final_max > curr_max
              ? curr_split_output_buffer_iter
              : curr_output_buffer;
  ...
  void partial_output(
      float* __restrict__ partial_output_buffer,
