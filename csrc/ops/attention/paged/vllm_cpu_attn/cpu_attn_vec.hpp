...
class TileGemm82 {
 public:
  ...
  static void gemm_micro(
      ...
      const int32_t dynamic_k_size,
      const bool accum_c) {
    static_assert(M > 0 && M <= 8, "M must be in the range 1-8 inclusive");
  ...
  static void copy_q_heads_tile(
    ...
    for (int32_t q_num_idx = 0; q_num_idx < q_num; ++q_num_idx) {
      for (int32_t q_head_idx = 0; q_head_idx < q_heads_per_kv; ++q_head_idx) {
        ...
        float* __restrict__ curr_q_buffer = q_buffer +
            q_num_idx * q_heads_per_kv * head_dim + q_head_idx * head_dim;
      ...
      for (int64_t head_idx = 0; head_idx < head_num; ++head_idx) {
        ...
        const int64_t block_offset = pos % block_size;
        {
          ...
          const scalar_t* key_start_ptr = key +
              token_idx * key_token_num_stride + head_idx * key_head_num_stride;
          scalar_t* key_cache_start_ptr = key_cache +
...
#pragma GCC unroll 8
        ...
        }
        {
          ...
          scalar_t* value_cache_start_ptr = value_cache +
              block_idx * num_blocks_stride + head_idx * cache_head_num_stride +
              block_offset * head_dim;
          std::memcpy(
              value_cache_start_ptr,
              value_start_ptr,
