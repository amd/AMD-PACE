...
template <typename T>
struct Vec {
  constexpr static int get_elem_num() {
    return T::VEC_ELEM_NUM;
  }
};
...
struct FP16Vec8 : public Vec<FP16Vec8> {
  ...
  void save(void* ptr) const {
    *reinterpret_cast<__m128i*>(ptr) = reg;
  }
};
...
struct FP16Vec16 : public Vec<FP16Vec16> {
  ...
  void save(void* ptr) const {
    _mm256_storeu_si256((__m256i*)ptr, reg);
  }
...
struct BF16Vec32 : public Vec<BF16Vec32> {
  ...
  explicit BF16Vec32(BF16Vec8& vec8_data)
      : reg((__m512i)_mm512_inserti32x4(
            _mm512_inserti32x4(
                _mm512_inserti32x4(
                    _mm512_castsi128_si512((__m128i)vec8_data.reg),
  ...
  void save(void* ptr) const {
    *reinterpret_cast<__m512i*>(ptr) = reg;
  }
};
#else
struct BF16Vec32 : public Vec<BF16Vec32> {
...
struct FP32Vec16 : public Vec<FP32Vec16> {
  ...
  explicit FP32Vec16(const FP32Vec4& data)
      : reg((__m512)_mm512_inserti32x4(
            _mm512_inserti32x4(
                _mm512_inserti32x4(
                    _mm512_castsi128_si512((__m128i)data.reg),
                    (__m128i)data.reg,
                    1),
                (__m128i)data.reg,
                2),
  ...
  explicit FP32Vec16(const FP32Vec8& data)
      : reg((__m512)_mm512_inserti32x8(
            _mm512_castsi256_si512((__m256i)data.reg),
            (__m256i)data.reg,
            1)) {}
  ...
  float reduce_max() const {
    return _mm512_reduce_max_ps(reg);
  }

  float reduce_min() const {
    return _mm512_reduce_min_ps(reg);
  }

  float get_last_elem() const {
    return _mm512_cvtss_f32(reg);
  }

  void save(float* ptr) const {
    _mm512_storeu_ps(ptr, reg);
  }
...
struct FP32Vec16 : public Vec<FP32Vec16> {
  ...
  FP32Vec16 operator*(const FP32Vec16& b) const {
    return FP32Vec16(
        _mm256_mul_ps(reg_low, b.reg_low), _mm256_mul_ps(reg_high, b.reg_high));
  }

  FP32Vec16 operator+(const FP32Vec16& b) const {
    return FP32Vec16(
        _mm256_add_ps(reg_low, b.reg_low), _mm256_add_ps(reg_high, b.reg_high));
  }

  FP32Vec16 operator-(const FP32Vec16& b) const {
    return FP32Vec16(
        _mm256_sub_ps(reg_low, b.reg_low), _mm256_sub_ps(reg_high, b.reg_high));
  }

  FP32Vec16 operator/(const FP32Vec16& b) const {
    return FP32Vec16(
        _mm256_div_ps(reg_low, b.reg_low), _mm256_div_ps(reg_high, b.reg_high));
  }

  FP32Vec16 max(const FP32Vec16& b) const {
    return FP32Vec16(
        _mm256_max_ps(reg_low, b.reg_low), _mm256_max_ps(reg_high, b.reg_high));
  }
...
inline FP16Vec16::FP16Vec16(const FP32Vec16& v)
    : reg(_mm256_insertf128_si256(
          ...
          FP16Vec8(FP32Vec8(v.reg_high)).reg,
          1)) {}
#endif
...
static void interleave_save(
    const BF16Vec16& vec0,
    const BF16Vec16& vec1,
    void* ptr) {
...
static void interleave_save(
    const FP16Vec16& vec0,
    const FP16Vec16& vec1,
    void* ptr) {
  __m512i vec_0 = _mm512_cvtepu16_epi32(vec0.reg);
  __m512i vec_1 = _mm512_cvtepu16_epi32(vec1.reg);
  vec_1 = _mm512_slli_epi32(vec_1, 16);
